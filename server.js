...
//! List all test files for a user
...
app.post('/submitTest', async (req, res) => {
  ...
  // Prepare the exercises object with actual details
...
//! Receive tests submitted by students
app.get('/recieveTest', async (req, res) => {
  const userName = req.query.userName; // Extract userName from query parameters
  const dirPath = path.join(__dirname, 'dbv1', userName, 'recieved_tests'); // Construct the path using userName

  try {
      const files = await fs.readdir(dirPath); // Read the directory contents
      ...
      res.status(200).json(receivedTests); // Respond with the received tests
  } catch (error) {
      console.error('Error reading received tests:', error); // Log any errors
      res.status(500).json({ message: 'Error retrieving received tests.' }); // Send an error response
  }
});

//! Dynamic endpoint to replace student name in the JSON file
app.post('/api/replace-student-name', async (req, res) => {
  const { testId, studentName } = req.body; // Extract testId and studentName from request body

  const filePath = path.join(__dirname, 'tests', `${testId}.json`); // Create the file path using the testId

  try {
      const data = await fs.readFile(filePath, 'utf-8'); // Read the existing content of the JSON file
      const jsonData = JSON.parse(data); // Parse the JSON data

      jsonData.studentName = studentName; // Replace the student name
      console.log('Updated JSON:', jsonData); // Log the updated JSON data

      await fs.writeFile(filePath, JSON.stringify(jsonData, null, 2)); // Write the updated content back to the JSON file
      res.status(200).json({ message: 'Student name replaced successfully', updatedData: jsonData }); // Respond with the updated JSON
  } catch (error) {
      console.error('Error replacing student name:', error); // Log any errors
      res.status(500).json({ error: 'Failed to replace student name' }); // Send an error response
  }
});
